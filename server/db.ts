...
for (const [index, name] of groupSeedValues.entries()) {
  db.prepare(
    `INSERT INTO groups (id, name, sort_order, is_active, created_at, updated_at)
...
db.prepare("UPDATE surveys SET group_id = ? WHERE group_id IS NULL OR TRIM(group_id) = ''").run(
  defaultGroupId
)
...
if (
  !tableContainsGeneralConstraint('surveys') ||
...
function hydrateGroup(row: any): SurveyGroup {
  return {
    id: row.id,
    name: row.name,
...
function hydrateSurveyDraft(row: any): SurveyDraft {
  return {
    id: row.id,
    survey_id: row.survey_id,
    survey_version_id: row.survey_version_id ?? null,
...
function hydrateSurveyVersion(row: any): SurveyVersion {
  return {
    id: row.id,
    survey_id: row.survey_id,
...
function hydrateSurveyTemplate(row: any): SurveyTemplate {
  return {
    id: row.id,
    name: row.name,
    description: row.description,
...
export const repo = {
  ...
  createGroup(name: string) {
    ...
    db.prepare(
      `INSERT INTO groups (id, name, sort_order, is_active, created_at, updated_at)
       VALUES (@id, @name, @sort_order, @is_active, @created_at, @updated_at)`
    ).run({
      ...payload,
  ...
  countSurveysByGroup(groupId: string) {
    const row = db
      .prepare('SELECT COUNT(*) as value FROM surveys WHERE group_id = ?')
      .get(groupId) as { value?: number } | undefined
  ...
  createTemplate(input: Omit<SurveyTemplate, 'id' | 'created_at' | 'updated_at'>) {
    const payload: SurveyTemplate = {
      ...input,
      id: nanoid(10),
      created_at: now(),
      updated_at: now(),
  ...
  updateTemplate(id: string, input: Omit<SurveyTemplate, 'id' | 'created_at' | 'updated_at'>) {
  ...
  createSurvey(
    input: Omit<Survey, 'id' | 'created_at' | 'updated_at' | 'group_id'> & {
      group_id?: string
      created_at?: string
      updated_at?: string
    }
  ...
  updateSurvey(
    id: string,
    input: Omit<Survey, 'id' | 'created_at' | 'updated_at' | 'group_id'> & { group_id?: string }
  ) {
  ...
  createResponse(
    ...
    db.prepare(
      `INSERT INTO survey_responses
       ...
       VALUES (@id, @survey_id, @survey_version_id, @survey_title, @survey_type, @respondent_name, @respondent_email, @submitted_at, @answers_json)`
    ).run({
      ...payload,
  ...
  getSurveyResponses(
    ...
    includeLegacyUnversioned = false
  ) {
    const rows = surveyVersionId
      ? (db
          .prepare(
            includeLegacyUnversioned
          ...
          .all(surveyId, surveyVersionId) as any[])
      : (db
  ...
  getSurveyDraft(surveyId: string, resumeToken: string) {
    const row = db
      .prepare(
        "SELECT * FROM survey_drafts WHERE survey_id = ? AND resume_token = ? AND status = 'draft'"
      )
      .get(surveyId, resumeToken) as any
    return row ? hydrateSurveyDraft(row) : null
  },
  createSurveyDraft(
    input: Omit<SurveyDraft, 'id' | 'resume_token' | 'created_at' | 'updated_at' | 'status'>
  ) {
    const timestamp = now()
    ...
    db.prepare(
      `INSERT INTO survey_drafts
       ...
       VALUES (@id, @survey_id, @survey_version_id, @resume_token, @respondent_name, @respondent_email, @current_stage, @current_page_index, @answers_json, @status, @created_at, @updated_at)`
    ).run({
      ...payload,
      answers_json: JSON.stringify(payload.answers),
    })
  ...
  updateSurveyDraft(
    ...
    input: Pick<
      SurveyDraft,
      'respondent_name' | 'respondent_email' | 'current_stage' | 'current_page_index' | 'answers'
    > & { survey_version_id?: string | null }
  ) {
  ...
  markSurveyDraftSubmitted(surveyId: string, resumeToken: string) {
    const existing = this.getSurveyDraft(surveyId, resumeToken)
    if (!existing) {
      return null
    }
...
export function seedRecentResponsesOnly(days = 14, surveyId?: string): SeedSummary {
  const surveys = repo
    .listSurveys()
    .filter(
      (survey) =>
